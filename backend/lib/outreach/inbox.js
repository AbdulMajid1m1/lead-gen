 ...
 * @returns array of { threadId, from, subject, date, messageId, headers,
 *   snippet, bounce } — `bounce` is null on a real reply, otherwise the
 *   classifyBounce result; `headers` is the raw header text fetched.
 */
export const findReplies = async ({ account, threads, since }) => {
  ...
  try {
    ...
    try {
      ...
      // auto-submitted, precedence and the x-auto* family are how an auto-
      // responder or a help desk declares itself (RFC 3834). The reply chain
      // is still read from in-reply-to/references alone.
      const headerBySeq = new Map();
      for await (const msg of client.fetch(range, {
        envelope: true,
        headers: [
          "in-reply-to", "references", "content-type", "return-path", "x-failed-recipients",
          "auto-submitted", "precedence", "x-autoreply", "x-autorespond", "x-auto-response-suppress",
          "x-autogenerated", "x-mailer", "x-zendesk-from-account-id", "x-freshdesk-ticket-id",
        ],
      })) {
        ...
        found.push({
          ...
          // Returned so the caller can run the auto-reply classifier on the
          // same evidence the bounce classifier saw.
          headers: headerText,
          snippet: null,
          bounce: null,
        });
